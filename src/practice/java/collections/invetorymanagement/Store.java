...
public class Store {
	...
	public static void main(String[] args) {
		...
		myStore.stockAisles();
		myStore.lisProductsByCategory();
		
		myStore.manageStoreCarts();
		
		myStore.lisProductsByCategory(false, true);
	...
	private void manageStoreCarts() {
		...
		if(!checkOutcart(cart3)) {
			System.out.println("Something went wrong, could not checkout");
		}
	...
	private void lisProductsByCategory() {
		
		lisProductsByCategory(true,false);
	
	}
	
	private void lisProductsByCategory(boolean inlcudeHeader,boolean includeDetail ) {
		
		aisleInventory.keySet().forEach(k -> {
			if(inlcudeHeader)System.out.println("-----------\n" + k + "\n------------");
			if(!includeDetail) {
