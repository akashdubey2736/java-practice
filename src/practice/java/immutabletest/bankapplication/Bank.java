...
public class Bank {
	...
	public void addCustomer(String name, double checkingInitialDeposit, double savingInitalDeposit) {
		BankCustomer customer= new BankCustomer(name,checkingInitialDeposit,savingInitalDeposit );
		...
		if(customer!=null) {
			...
			if(account != null) {
				if((account.getBalance() + amount)<0) {
					...
					account.commitTransactions(routingNumber, id, lastTransactionId++, amount);
					return true;
				}
			}
